...
def decrypt_token(encrypted_token: str) -> str:
    ...
    try:
        ...
        if not key:
            logger.warning("No encryption key available, assuming token is plaintext")
        ...
        logger.warning(f"Error decrypting token (may be plaintext): {e}")
...
def get_federated_token(session_id: str):
    """
    Get the federated token from session data and decrypt it.
    
    The federated token is used to make API calls to external services like Okta
    on behalf of the authenticated user.
    """
    session_data = get_session_data(session_id)
    if session_data and session_data.get('federated_token'):
        encrypted_token = session_data.get('federated_token')
